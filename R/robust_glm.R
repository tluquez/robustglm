...
predict.robust_glm <- function(object, newdata = NULL,
  ...
  preds <- predict(object, newdata = newdata, type = type, se.fit = se.fit, ...)

  if (se.fit) {
    if (sandwich && !is.null(object$sandwich)) {
      se <- sqrt(diag(model.matrix(object$formula, newdata) %*%
                        object$sandwich[, "std.error"]^2))
      preds$se.fit <- se
    }
  }
